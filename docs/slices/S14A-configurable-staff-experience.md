...
ITF Flow preserves its Classic staff interface and adds a structurally different Modern experience.
A system administrator can privately preview either mode and activate one organization-wide without a
deployment. Both modes use the same authorization-aware navigation and dashboard data.

## Delivered scope

- `StaffUiMode` supports `CLASSIC` and `MODERN`, with Classic as the migration default.
